...
exports.login = async (req, res, next) => {
    try {
        ...
        try {
            // Find admin user by email in members table
            const [users] = await pool.execute(
                'SELECT id, email, first_name, last_name, password, role FROM members WHERE email = ? AND role = "admin"',
                [username]
            );
            ...
            // Update last login (optional - we can skip this for members table)
            // await pool.execute(
            ...
            res.json({
                ...
                user: {
                    id: user.id,
                    username: user.email, // Use email as username for members
                    email: user.email,
